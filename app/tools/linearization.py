...
def get_linearized_matrices():
    ...
    # Define denominator and numerator from the non-linear equations
    ...
    # Helper to convert sympy matrix to list of strings
    def matrix_to_string_list(matrix):
        return [[str(element) for element in row] for row in matrix.tolist()]

    return {
        "A": matrix_to_string_list(A_eq),
        "B": matrix_to_string_list(B_eq),
        "B_d": matrix_to_string_list(Bd_eq)
    }

if __name__ == "__main__":
    import json
    matrices = get_linearized_matrices()
